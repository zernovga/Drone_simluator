...
def drawText(x, y, text):                                                
    textSurface = font.render(text, True, 
                              (255, 255, 255), (255, 0, 0))
    textData = pygame.image.tostring(textSurface, "RGB", True)
    glGenTextures( 1, textData)
    glBindTexture( GL_TEXTURE_2D, textData )
    glWindowPos2d(x, y)
    glDrawPixels(textSurface.get_width(), textSurface.get_height(), 
                 GL_RGB, GL_UNSIGNED_BYTE, textData)
